...
git pull git@github.com:Mic92/flake-fmt main
...
# refresh Cargo.lock so the recorded crate versions match the new release
cargo update --workspace --offline
git add Cargo.toml Cargo.lock package.nix bin/create-release.sh
git branch -D "release-${version}" || true
git checkout --no-track -b "release-${version}"
git commit -m "bump version ${version}"
# explicit refspec so push.default=upstream cannot redirect this to main
git push -f origin "release-${version}:release-${version}"
pr_url=$(gh pr create \
  --base main \
  --head "release-${version}" \
  --title "Release ${version}" \
  --body "Release ${version} of flake-fmt")
